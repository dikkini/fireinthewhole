...
class GameScene: SKScene {
    ...
    override func touchesEnded(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if selectedTile is Character && (self.selectedCharacter == nil || selectedTile == self.selectedCharacter) {
        ...
        } else if selectedTile is Character && self.selectedCharacter !== nil
                          && selectedTile as? Character !== self.selectedCharacter {
        ...
        } else if selectedTile is Ground && self.selectedCharacter !== nil && self.selectedGround == nil {
        ...
        } else if selectedTile is Ground && self.selectedGround != nil && self.selectedCharacter != nil {
            //если выбрана земля и до этого была выбрана земля (которая уже подсвечена)
            self.hideCharacterActionsMenu()
            if selectedTile?.position == self.selectedGround?.position {
                // и координаты совпадают, то надо идти в эту точку
                if self.selectedCharacter?.action == TileAction.Move {
                    return
                }
                self.selectedCharacter?.action = TileAction.Move
                ...
                if path != nil {
                    ...
                    self.selectedCharacter!.move(path: path!, completion: { newTileIndexOfChar in
                        ...
                        self.tileService.updateTileMap(oldIndex: oldIndex, newIndex: newIndex, oldType: oldType,
                                newType: newType)
                        self.selectedCharacter?.action = TileAction.Idle
                    })
                } else {
                ...
                if (self.selectedCharacter?.canMoveTo(point25D: (selectedTile?.position)!))! {
                    // иначе либо подсветить другую землю
                    movePointHighlight?.position = (selectedTile?.position)!
                    self.selectedGround = selectedTile as? Ground
                } else {
                    // либо убрать подсветку патенциального хода
